...
SECTOR_OUTLOOK = \{\
    ...
    "Power":        ("Strong",   "India power demand surging \'97 summer heat, data-centre expansion, EV growth."),\
    "IT":           ("Cautious", "Sector beaten down \'97 contrarian opportunity; weak rupee boosts USD revenue."),\
...
def is_market_open():\
    ...
    return mo <= now <= mc\
\
# \uc0\u9472 \u9472  Technical indicators \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
\
def ema(data, span):\
    k, r = 2 / (span + 1), [data[0]]\
...
\
# \uc0\u9472 \u9472  Scoring engine \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
\
def score_stock(d):\
    score, reasons = 0, []\
\
    ...
    rsi = d.get("rsi")\
    if rsi is not None:\
        if rsi < 30:   score += 3; reasons.append(f"RSI \{rsi\} \'97 deeply oversold, strong bounce potential")\
        elif rsi < 45: score += 2; reasons.append(f"RSI \{rsi\} \'97 oversold, bullish setup forming")\
        elif rsi < 60: score += 1; reasons.append(f"RSI \{rsi\} \'97 neutral with upward bias")\
        elif rsi > 75: score -= 2; reasons.append(f"RSI \{rsi\} \'97 overbought, caution warranted")\
        else:          score -= 1; reasons.append(f"RSI \{rsi\} \'97 slightly elevated")\
\
    # MACD\
    hist = d.get("macd_hist")\
    macd_v, sig_v = d.get("macd"), d.get("macd_signal")\
    if hist is not None:\
        if hist > 0 and macd_v > sig_v:   score += 2; reasons.append("MACD bullish crossover \'97 momentum turning positive")\
        elif hist > 0:                     score += 1; reasons.append("MACD histogram positive \'97 mild bullish momentum")\
        elif hist < 0 and macd_v < sig_v:  score -= 2; reasons.append("MACD bearish \'97 downward momentum dominant")\
        else:                              score -= 1; reasons.append("MACD histogram negative \'97 caution")\
\
    # Moving averages\
    c = d["current_price"]; ma20, ma50, ma200 = d["ma20"], d["ma50"], d["ma200"]\
    if c > ma50 > ma200:    score += 3; reasons.append("Price above MA50 & MA200 \'97 confirmed uptrend")\
    elif c > ma50:          score += 2; reasons.append("Price above MA50 \'97 medium-term bullish")\
    elif c > ma200:         score += 1; reasons.append("Price above MA200 \'97 long-term support holding")\
    elif c < ma50 < ma200:  score -= 2; reasons.append("Price below MA50 & MA200 \'97 downtrend in play")\
    if ma20 > ma50:         score += 1; reasons.append("MA20 above MA50 \'97 short-term acceleration")\
\
    # Volume\
    vr = d.get("volume_ratio", 1)\
    if vr > 2.0:    score += 2; reasons.append(f"Volume \{vr\}x avg \'97 strong institutional interest")\
    elif vr > 1.3:  score += 1; reasons.append(f"Volume \{vr\}x avg \'97 above-average participation")\
    elif vr < 0.5:  score -= 1; reasons.append("Low volume \'97 weak conviction")\
\
    # Bollinger Bands\
    bb_l, bb_m, bb_h = d.get("bb_low"), d.get("bb_mid"), d.get("bb_high")\
    if bb_l and bb_h:\
        if c < bb_l:         score += 2; reasons.append("Price below lower Bollinger Band \'97 mean-reversion buy signal")\
        elif c > bb_h:       score -= 1; reasons.append("Price above upper Bollinger Band \'97 stretched, wait for pullback")\
        elif c < bb_m:       score += 1; reasons.append("Price below Bollinger midline \'97 room to expand upward")\
\
    # 52-week position\
    pct = d.get("pct_from_52w_high", 0)\
    if pct < -30:   score += 2; reasons.append(f"Stock is \{abs(pct)\}% below 52-week high \'97 deep value zone")\
    elif pct < -15: score += 1; reasons.append(f"Stock is \{abs(pct)\}% below 52-week high \'97 discounted entry")\
    elif pct > -5:  score -= 1; reasons.append("Near 52-week high \'97 limited upside headroom short term")\
\
    # Intrinsic value\
    ...
    if iv_status == "Undervalued":\
        ...
        reasons.append(f"Intrinsic value \uc0\u8377 \{d.get('intrinsic_value')\} \'97 stock undervalued by \{mos\}%")\
    elif iv_status == "Overvalued":\
        score -= 2\
        reasons.append(f"Trading above intrinsic value by \{abs(mos or 0)\}% \'97 valuation risk")\
\
    return score, reasons\
\
# \uc0\u9472 \u9472  Recommendation builder \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
\
...
\
    ...
    mos = d.get("margin_of_safety")\
    iv_str = f"\uc0\u8377 \{iv\} (\{d.get('iv_status')\}; \{mos:+.1f\}% vs CMP)" if iv else "Data unavailable"\
\
    # Technical summary\
    ...
    tech_sum = (\
        ...
        f"MACD histogram \{'positive \'97 bullish momentum' if macd_h and macd_h > 0 else 'negative \'97 caution'\}. "\
        f"Price is \{d['trend'].lower()\} (MA20 \uc0\u8377 \{d['ma20']\}, MA50 \u8377 \{d['ma50']\}). "\
        f"Volume at \{d.get('volume_ratio',1)\}x 20-day average. "\
        f"Bollinger midline \uc0\u8377 \{d.get('bb_mid')\}."\
    )\
\
    # Fundamental\
    sector = d["sector"]\
    fund_sum = (\
        ...
        f"52-week range: \uc0\u8377 \{d['week52_low']\}\'96\u8377 \{d['week52_high']\}; "\
...
\
    ...
    sit_sum = (\
        ...
        f"GST 2.0 reforms, rupee at \uc0\u8377 95+ (boosts export-linked sectors)."\
    )\
\
    # Risk\
    risk_notes = []\
    if d.get("beta", 1) > 1.2: risk_notes.append("high beta stock \'97 amplifies market swings")\
    if d.get("rsi", 50) > 70:  risk_notes.append("overbought RSI \'97 pullback risk")\
    if d.get("pct_from_52w_high", -10) > -5: risk_notes.append("near 52-week high \'97 limited upside headroom")\
...
\
    ...
    holding = "6\'968 weeks" if signal in ("BUY", "SELL") else "Monitor 2\'964 weeks"\
\
    return \{\
        "rank": rank,\
        "name": d["name"],\
...
\
    ...
    if opt_type == "CE":\
        ...
        if strike >= c: strike -= 50\
\
    # Expiry \'97 pick next month or month after for 2-3 month horizon\
    now = datetime.now(IST)\
...
\
    if opt_type == "CE":\
        reasoning = (\
            ...
            f"Expected move of ~\{expected_move\}% in underlying over 4\'966 weeks. "\
            f"\{SECTOR_OUTLOOK.get(stock_data['sector'], ('', ''))[1]\} "\
            f"Use Jun/Jul expiry to avoid near-term theta burn \'97 VIX elevated at ~19."\
        )\
    else:\
        reasoning = (\
...
\
    ...
    \}\
\
# \uc0\u9472 \u9472  Data fetcher \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
\
...
\
        ...
        return None\
\
# \uc0\u9472 \u9472  Market summary \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
\
def generate_market_summary(stocks):\
    ...
    return (\
        ...
        f"Key macro watch: RBI June meeting, crude oil above $105, rupee near \uc0\u8377 95.7."\
    )\
\
# \uc0\u9472 \u9472  Routes \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
\
@app.route("/")\
def index():\
    try:\
...
\
        if len(stocks_data) < 5:\
            return jsonify(\{"error": "Could not fetch enough market data. Yahoo Finance may be rate-limiting \'97 wait 1 minute and retry."\}), 500\
\
